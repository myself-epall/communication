...
void get_uid(myevent_s *ev)
{
    ...
    FILE *fp = fopen("/home/Luo/communication/user_msg", "a+");
    if(fp == NULL) 
    {
        write(ev->fd, "error\n", 6);
...
void cb_write(int cfd, void *arg)
{
	...
	int i;
    for(i = r[0]; i != 1; i = r[i])             // 遍历当前的在线链表, 向在线用户发送
    {
...
void load_usermsg()                 
{
    FILE *fp = fopen("/home/Luo/communication/user_msg", "r");         // 打开保存已注册用户信息的文件
...
int main(int argc, char *argv[])
{
    ...
    while(1)
    {
        ...
        for(k = 0; k < num; ++k)
        {
            myevent_s *ev = (myevent_s*)eps[k].data.ptr;
            ev->fun(ev->fd, ev->arg);
        }
    }
    close(lfd);
